...
impl<Message, P: Program<Message>> BackgroundShader<Message, P> {
    /// Create a new custom [`Shader`].
    pub fn new(program: P) -> Self {
        Self {
            width: Length::Fixed(100.0),
...
impl<P, Message, Theme, Renderer> Widget<Message, Theme, Renderer>
    for BackgroundShader<Message, P>
where
    P: Program<Message>,
    Renderer: primitive::Renderer,
{
...
impl<'a, Message, Theme, Renderer, P> From<BackgroundShader<Message, P>>
    for Element<'a, Message, Theme, Renderer>
where
    Message: 'a,
    Renderer: primitive::Renderer,
...
{
    fn from(
        custom: BackgroundShader<Message, P>,
    ) -> Element<'a, Message, Theme, Renderer> {
        Element::new(custom)
    }
}
